...
def get_licence_details(logfile, folder) -> List[Dict[str, object]]:
    ...
    projects = [f for f in folder.iterdir() if f.is_dir()]
    for i, project in enumerate(projects):
        copyright_path = folder / project / "copr.htm"
        log_and_print(
            logfile,
            f"Extracting licence information for {project} from {copyright_path}",
        )
...
def rename_usfm(project_dir: Path, logfile: Path):
    """
    Renames the .usfm files within a project to a standard format by removing the leading 3 characters in some cases.
    ...
    for old_usfm_path in usfm_paths:
        ...
        log_and_print(logfile, f"Renaming {old_usfm_path}     to     {new_sfm_path}")
        old_usfm_path.rename(new_sfm_path)
        # os.rename(old_usfm_path, new_sfm_path)
...
def main() -> None:
    ...
    parser.add_argument(
        "--base-folder",
        default=None,
        help="Override the base folder location (defaults to EBIBLE_DATA_DIR from .env or '_ebible_data').",
    )
    ...
    if args.base_folder:
        ...
        print(f"Using base folder from command line argument: {base}")
    elif os.getenv("EBIBLE_DATA_DIR"):
        base = Path(os.getenv("EBIBLE_DATA_DIR"))
        print(f"Using base folder from EBIBLE_DATA_DIR environment variable: {base}")
    else:
        # Define a sensible default relative to the script or repo root
        # Assuming ebible.py is in BibleNLP/ebible/code/python/
    ...
    if args.force_download:
        # Only wipe data in the projects_folder and private_projects_folder if the --force_download option is given.

        for dir in tqdm(enumerate[projects_folder, private_projects_folder]):
    ...
    # unzip them and add a Settings.xml file within each
    log_and_print(logfile, f"Unzipping projects to: {projects_folder}")

    # Get the list of project folders to process
    for translation_id in tqdm(translation_ids):
        ...
        # Unzip it only if it doesn't exist, or if for the--force-download option is given.
        if project_dir.exists() and not args.force_download:
            log_and_print(logfile, f"{project_dir} exists, not unzipping again.")
            continue
        else:
            log_and_print(logfile, f"Unzipping {translation.id} to: {project_dir}")
            ...
            try:
                ...
                log_and_print(
                    logfile, f"ReadError: While trying to unzip: {project_dir}"
                )
        ...
        # Pass the loaded data into the function
        write_settings_file(
            project_dir, translation.language_code, translation_id, vrs_diffs_data
        )
